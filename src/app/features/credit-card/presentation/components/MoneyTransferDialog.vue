...
async function submit() {
  ...
  try {
    ...
    if (isFormCorrect) {
      await objStore.transferMoney(state)
      toast.add({
        severity: "success",
        summary: t('labels.operationSuccess'),
        detail: t('features.cc.transferMoneySuccessDesc'),
        life: 10000
      })
...
<template>
  <Dialog v-model:visible="model" modal :header="$t('labels.transferMoney')" :closable="false">
    <form @submit.prevent="submit" class="mt-6 flex flex-col gap-8">
      <NikkInputText
        v-model="state.recipientCardNumber"
        errorHelpLabel="errors.validation.requiredField"
        id="tm-card-number"
        :isError="v$.recipientCardNumber.$error"
        label="labels.recipientCardNumber"
        type="text"
      />
      ...
      <div class="flex place-content-between mt-4">
        ...
        <PrimeButton :loading="loading" type="submit" :label="$t('labels.transfer')" />
      </div>
    </form>
  </Dialog>
</template>
