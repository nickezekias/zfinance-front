...
interface Props {
  ...
  registerRouteName?: string
}
const props = withDefaults(defineProps<Props>(), {
  isRegisterAction: true,
  forgotPasswordRouteName: 'auth.forgotPassword',
...
async function submit(): Promise<void> {
  ...
  try {
    ...
    if (isFormCorrect) {
      await props.authStore.login(state)
...
<template>
  <form @submit.prevent="submit">
    ...
    <div class="flex">
      <router-link class="ml-auto" :to="{ name: props.forgotPasswordRouteName }">
    ...
    <div class="actions mt-8">
      ...
      <router-link v-if="props.isRegisterAction" :to="{ name: props.registerRouteName }">
